...
int fork(void) {
  // 1. 遍历进程数组
  // 2. 当前进程，如果进程状态不是未使用，continue
  // 3. 初始化子进程。
  //   a. 子进程状态可运行。
  //   b.
  //   将当前进程的进程切换上下文、进程中断上下文、进程打开文件数组、进程用户态页表拷贝到子进程。
  //   c. 使用虚拟内存管理器的拷贝进程用户态页表到子进程用户态页表
  //   d.
  //   对当前进程打开文件数组的每个打开文件，使用文件管理器的增加打开文件引用计数
...
uint64 count_process(void) {
  // 1. 遍历进程数组，累计进程状态不是未使用的进程数量

  // added function for counting used process slots (lab2)
  uint64 cnt = 0;
  for (struct proc *p = proc; p < &proc[NPROC]; p++) {
    // acquire(&p->lock);
    // no need to lock since all we do is reading, no writing will be done to
    // the proc.
    if (p->state != UNUSED) {
      cnt++;
    }
