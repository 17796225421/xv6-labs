...
uint64 sys_open(void) {
  // 1. 使用系统调用管理器的获取参数，得到路径和模式
  // 2.
  // 根据模式判断是否是符号链接，如果不是，使用文件管理器的根据路径分配inode，获得路径的inode
  // 3. 如果是符号链接？？
...
uint64 sys_symlink(void) {
  // 1. 使用系统调用管理器的获取参数，得到源路径和目标路径
  // 2. 使用文件管理器的根据路径分配inode，获得源路径的inode
  // 3.
  // 使用文件系统管理器的写inode，将目标路径写入源路径的inode的块数组的第0个块
