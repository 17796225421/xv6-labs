...
uint64 sys_mmap(void) {
  // 1. 系统调用管理器的获取参数
  // 2. 遍历映射信息数组，找到一个映射信息标记无效
  // 3. 将该映射信息标记有效，设置映射信息起始地址、大小、文件、文件内偏移、权限
  // 4. 使用文件管理器的增加文件引用计数
  // 5. 返回映射信息的起始地址
...
struct vma *findvma(struct proc *p, uint64 va) {
  // 1.
  // 遍历映射信息数组，找到映射信息标记有效的，起始地址和大小包含要寻找的虚拟地址
...
uint64 sys_munmap(void) {
  // 1. 系统调用管理器的获取参数
  // 2. 文件管理器的查找映射信息，将虚拟地址对应的映射信息找到
  // 3. 使用虚拟内存管理器的释放物理页并删除映射并回写
...
int vmatrylazytouch(uint64 va) {
  ...
  begin_op();
  ilock(v->f->ip);
  readi(v->f->ip, 0, (uint64)pa, v->offset + PGROUNDDOWN(va - v->vastart),
        PGSIZE);
  iunlock(v->f->ip);
  end_op();
