...
void vmaunmap(pagetable_t pagetable, uint64 va, uint64 nbytes, struct vma *v) {
  // 1. 使用虚拟地址管理器的虚拟页虚拟地址找到物理页物理地址
  // 2.
  // 如果最后一级页表的页表项标记mmap且标记脏页，使用文件系统管理器的写inode。
  // 3. 使用物理内存管理器的回收物理页
  // 4. 删除映射
  ...
  for (a = va; a < va + nbytes; a += PGSIZE) {
    if ((pte = walk(pagetable, a, 0)) == 0) panic("sys_munmap: walk");
    if (PTE_FLAGS(*pte) == PTE_V) panic("sys_munmap: not a leaf");
    if (*pte & PTE_V) {
      uint64 pa = PTE2PA(*pte);
      if ((*pte & PTE_D) &&
          (v->flags & MAP_SHARED)) {  // dirty, need to write back to disk
        begin_op();
        ilock(v->f->ip);
        uint64 aoff =
            a - v->vastart;  // offset relative to the start of memory range
        if (aoff < 0) {      // if the first page is not a full 4k page
          writei(v->f->ip, 0, pa + (-aoff), v->offset, PGSIZE + aoff);
        } else if (aoff + PGSIZE >
                   v->sz) {  // if the last page is not a full 4k page
